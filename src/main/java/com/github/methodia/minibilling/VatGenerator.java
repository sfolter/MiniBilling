...
public class VatGenerator {
    ...
    public List<Vat> generate(List<BigDecimal> vatPercentages, List<InvoiceLine> invoiceLines, List<Tax> taxes) {
    ...
    private List<Vat> invoiceLinesVatGenerator(List<BigDecimal> vatPercentages, List<InvoiceLine> invoiceLines) {
        List<Vat> vatInvoiceLines = new ArrayList<>();
        for (InvoiceLine invoiceLine : invoiceLines) {
            List<Integer> vattedLines = new ArrayList<>();
            vattedLines.add(invoiceLine.getIndex());

            for (BigDecimal percentage : vatPercentages) {
                BigDecimal vatAmount = invoiceLine.getAmount().multiply(percentage).divide(new BigDecimal("100"))
                        .setScale(2, RoundingMode.HALF_UP).stripTrailingZeros();
                vatInvoiceLines.add(
                        new Vat(vatInvoiceLines.size() + 1, vattedLines, String.valueOf(percentage), vatAmount));
            }
        }
    ...
    private List<Vat> taxVatGenerator(int index, List<Tax> taxes) {
        ...
        for (Tax tax : taxes) {
            ...
            vatTax.add(new Vat(index + vatTax.size() + 1, String.valueOf(percentage), vatAmount, taxedLines));
        }
        return vatTax;
    }
}
