...
public class Main {
    ...
    public static void main(final String[] args) {
        ...
        try (session) {
            final HDBUserReader hdbUserReader = new HDBUserReader(session);
            final Map<String, User> userList = hdbUserReader.read();
            ...
            for (final Map.Entry<String, User> userFromMap : userList.entrySet()) {
                ...
                final DBSave dbSave = new DBSave(session, invoice);
                dbSave.save();
            }
        } catch (Throwable t) {
            if (session.isOpen()) {
