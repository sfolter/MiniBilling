    ...
    public Invoice generate() throws IOException, ParseException {
        ...
        for (QuantityPricePeriod qpp : distribute) {
            ...
            if (yearMonthLocalDate.compareTo(end) >= 0) {
                ...
                BigDecimal amountInVat1 = amount.multiply(new BigDecimal(TAXED_AMOUNT_PERCENTAGE_VAT_1).divide(new BigDecimal("100"),2,RoundingMode.HALF_UP))
                        .multiply(new BigDecimal(PERCENTAGE)).divide(new BigDecimal(100),2,RoundingMode.HALF_UP);

                BigDecimal amountInVat2 = amount.multiply(new BigDecimal(TAXED_AMOUNT_PERCENTAGE_VAT_2).divide(new BigDecimal("100"),2,RoundingMode.HALF_UP))
                ...
                vatLines.add(new VatLine(indexInVat+++1, index, 0, TAXED_AMOUNT_PERCENTAGE_VAT_2,  PERCENTAGE-10, amountInVat2));
