...
public class InvoiceGenerator {
    ...
    public Invoice generate(final User user, final Collection<Measurement> measurements, final Collection<Price> prices,
                            final String yearMonthStr, final String currency) throws IOException, ParseException {
        ...
        for (final QuantityPricePeriod qpp : quantityPricePeriods) {
            ...
            if (0 <= yearMonthLocalDate.compareTo(end)) {
                ...
                final long daysQuantity = start.toLocalDate().atStartOfDay()
                        .until(end.toLocalDate().atTime(23, 59), ChronoUnit.DAYS);
                priceForTaxes = priceForTaxes.multiply(currencyRate).setScale(2, RoundingMode.HALF_EVEN);
                final BigDecimal amountForTaxes = priceForTaxes.multiply(BigDecimal.valueOf(daysQuantity))
                        .setScale(2, RoundingMode.HALF_EVEN);

                BigDecimal vatAmount1 = amount.multiply(taxedAmountPercentage1)
                        .divide(BigDecimal.valueOf(100), 2, RoundingMode.HALF_EVEN);
                vatAmount1 = vatAmount1.multiply(VAT_PERCENTAGE)
                        .divide(BigDecimal.valueOf(100), 2, RoundingMode.HALF_EVEN);
                BigDecimal vatAmount2 = amount.multiply(taxedAmountPercentage2)
                        .divide(BigDecimal.valueOf(100), 2, RoundingMode.HALF_EVEN);
                vatAmount2 = vatAmount2.multiply(SECOND_VAT_PERCENTAGE)
                        .divide(BigDecimal.valueOf(100), 2, RoundingMode.HALF_EVEN);
                ...
                totalAmountWithVat = totalAmountWithVat.add(vatAmount1).add(vatAmount2).add(vatAmount3)
                        .add(totalAmount);

                invoiceLines.add(new InvoiceLine(lineIndex, quantity, start, end, product, price, priceList, amount));
                taxesLines.add(new TaxesLine(lineIndex, lineIndex, NAME, daysQuantity, UNIT_DAYS, priceForTaxes,
                        amountForTaxes));
                vatLines.add(new VatLine(vatIndex, lineIndex, 0, taxedAmountPercentage1, VAT_PERCENTAGE, vatAmount1));
                vatLines.add(new VatLine(vatIndex++ + 1, lineIndex, 0, taxedAmountPercentage2, SECOND_VAT_PERCENTAGE,
                        vatAmount2));
                vatLines.add(
                        new VatLine(vatIndex++ + 1, lineIndex, 1, taxedAmountPercentage3, VAT_PERCENTAGE, vatAmount3));
            }
        }
